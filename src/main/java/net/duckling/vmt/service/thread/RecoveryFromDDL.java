 ...
 * @since 2013-5-31
 */
public class RecoveryFromDDL implements Jobable{
	 ...
	 * @param umtId 触发者的id
	 */
	public RecoveryFromDDL(String umtId){
	...
	private void syncATeam(String teamCode){
		...
		if (destGroup==null){
			...
			ConsoleController.write("it is can find by ddl["+teamCode+"] ,doing...");
	...
	public void doJob() {
		...
		for (String teamCode : groupTeamCodes) {
			try{
				ConsoleController.write("["+(index++)+"/"+count+"] doing");
				syncATeam(teamCode);
			}catch(Exception e){
				ConsoleController.write(e);
			}
		}
		ConsoleController.write("recovery from ddl action is over!");
		ConsoleController.writeOver();
	}

	@Override
	public String getJobId() {
		return DDL_JOB+".recovery.from.ddl";
	}
	@Override
